# Per lanciare allocator
#reallocated_spend = process_budget_allocator_results_data(budget_allocator, mmm_dashboard_config)
import datetime
import pandas as pd
import numpy as np
import math
import warnings
from cassandra_functions_3 import create_summary_dictionary, apply_adstock_transformation,\
                                apply_decomp_transformation, apply_saturation_transformation


def process_budget_allocator_results_data(budget_allocator, mmm_dashboard_config):
    """
    Process the budget allocator results data.

    Args:
        budget_allocator (object): The budget allocator object.
        mmm_dashboard_config (object): The MMM dashboard configuration object.

    Returns:
        dict: A dictionary containing the processed and ordered reallocated spend data.
    """
    ...
    mmm_dashboard_config['df'] = mmm_dashboard_config['df'].loc[mmm_dashboard_config['df'][mmm_dashboard_config['date_var']] <= allocation_dates['date_max']]
    
    general_info = setup_general_info(budget_allocator, mmm_dashboard_config)
    
    historical_info = setup_historical_info(mmm_dashboard_config)
    
    initial_info = setup_initial_info(budget_allocator, mmm_dashboard_config)
    
    expected_info = setup_expected_info(budget_allocator, mmm_dashboard_config, initial_info['initSpendUnitTotal'][0])
    
    optimal_info = setup_optimal_info(budget_allocator, mmm_dashboard_config, historical_info['histSpendUnit'], expected_info['expSpendUnitTotal'][0], initial_info['initSpendUnit'], initial_info['initSpendUnitTotal'][0], initial_info['initResponseUnit'], initial_info['initResponseUnitTotal'][0])

    columns_order= ["channels", "solID", "dep_var_type", "date_min", "date_max", "periods", "constr_low",
                                                "constr_up", "histSpend", "histSpendUnit", "histSpendTotal", "histShareSpend",
                                                    "initSpendUnitTotal", "initSpendUnit", "initSpendShare", "initResponseUnit",
                                                    "initResponseUnitTotal", "initRoiUnit", "expSpendTotal", "expSpendUnitTotal",
                                                    "expSpendUnitDelta", "optmSpendUnit", "optmSpendUnitDelta",
                                                    "optmSpendUnitTotal", "optmSpendUnitTotalDelta", "optmSpendShareUnit",
                                                    "optmResponseUnit", "optmResponseUnitTotal", "optmRoiUnit",
                                                    "optmResponseUnitLift", "optmResponseUnitTotalLift"]
    
    reallocated_spend = {**general_info, **historical_info, **initial_info, **expected_info, **optimal_info}
    ordered_reallocated_spend = {key: reallocated_spend[key] for key in columns_order}
    
    return ordered_reallocated_spend

def setup_optimal_info(budget_allocator,
                       mmm_dashboard_config,
                       hist_spend_unit,
                       exp_pend_unit_total,
                       init_spend_unit,
                       init_spend_unit_total,
                       init_response_unit,
                       init_response_unit_total):
    """
    ...
    periods_in_days, expected_spend_days_by_type_aggregation, periods_to_estimate, data_expected_spend = adjust_total({'budget_start':allocation_dates['date_min'],
                                                                                                                    'budget_end':allocation_dates['date_max'],
                                                                                                                    'expected_spend':budget_allocator.expected_spend},
                                                                                                                    mmm_dashboard_config['type_aggregation'])
    
    new_dates = extend_date_range(mmm_dashboard_config['df'], mmm_dashboard_config['date_var'], periods_to_estimate, mmm_dashboard_config['type_aggregation'], allocation_dates['date_max'])
    ...
    optimized_spend = find_optimized_spend(budget_allocator, mmm_dashboard_config, periods_to_estimate, exp_pend_unit_total, init_spend_unit, rollingWindowLength)
    
    summary_dict = create_summary_dictionary(mmm_dashboard_config['all_ind_vars'], mmm_dashboard_config['hypers'], mmm_dashboard_config['coefs'])
    
    df_optim = append_optimal_spend({'df': mmm_dashboard_config['df'],
                                                  'paid_media':mmm_dashboard_config['paid_media'], 'date_var':mmm_dashboard_config['date_var']}, optimized_spend, periods_to_estimate, allocation_dates['date_max'])
    
    
    df_optim[mmm_dashboard_config['date_var']] = new_dates

    df_optim_adstock = apply_adstock_transformation(df_optim, mmm_dashboard_config['paid_media'], summary_dict, mmm_dashboard_config['date_var'])
    
    df_optim_saturation, df_optim_adstock_filtered = apply_saturation_transformation(df_optim_adstock, mmm_dashboard_config['paid_media'], summary_dict, mmm_dashboard_config['date_var'], mmm_dashboard_config['window_start'], new_dates[-1])
    
    df_optim_decomp = apply_decomp_transformation(df_optim_saturation, mmm_dashboard_config['paid_media'], summary_dict, mmm_dashboard_config['date_var'])

    df_optim_decomp_reallocated_spend = df_optim_decomp.tail(periods_to_estimate)
    
    optm_spend_unit_delta = [(optm / hist) - 1 if hist != 0 else -1 for optm, hist in zip(optimized_spend, hist_spend_unit)]
    ...
    optm_roi_unit = [response / spend if int(spend) != 0 else 0 for response, spend in zip(optm_response_unit, optimized_spend)]
    optm_response_unit_lift = [(optm / init) - 1 if not math.isnan(init) and int(init) != 0 else -1 for optm, init in zip(optm_response_unit, init_response_unit)]
...
def find_optimized_spend(budget_allocator, mmm_dashboard_config, periods_to_estimate, exp_spend_unit_total, init_spend_unit, rollingWindowLength):
    """
    ...
    channelConstrMeanSorted = [(low + up) / 2 for low, up in zip(budget_allocator.channel_constr_low_sorted, budget_allocator.channel_constr_up_sorted)]
    
    paid_media_spends = [channelConstrMeanSorted[i] if spend < budget_allocator.channel_constr_low_sorted[i] or spend > budget_allocator.channel_constr_up_sorted[i] else spend for i, spend in enumerate(init_spend_unit)]
    ...
    paid_media_spend_traspose = mmm_dashboard_config['df'][mmm_dashboard_config['paid_media']].transpose().values.tolist()
    
    target_value = budget_allocator.target_value if budget_allocator.target_value else None
    
    budget_allocator_spend = budget_allocation(paid_media_spends, budget_allocator.channel_constr_low_sorted, budget_allocator.channel_constr_up_sorted, periods_to_estimate, budget_allocator.maxeval, budget_allocator.xtol_rel, eval_list, paid_media_spend_traspose, rollingWindowLength, target_value, mmm_dashboard_config['response'])
...
def calculate_allocation_dates(budget_allocator, mmm_dashboard_config, df_budget_allocator=pd.DataFrame()):
    ...
    if df_budget_allocator.empty:
        ...
        date_max = adjust_date(datetime.strptime(mmm_dashboard_config['window_end'], '%Y-%m-%d'), mmm_dashboard_config['type_aggregation'])
        date_min = calculate_start_date_based_on_aggregation(date_max, expected_spend_days, mmm_dashboard_config['type_aggregation'])
    else:
    ...
    dates = adjust_allocation_dates_for_aggregation(date_min, date_max, budget_allocator.expected_spend_days, mmm_dashboard_config['type_aggregation'])
    
    # Format and return dates
    return format_dates(dates['date_min'], dates['date_max'], dates['date_allocation_min'], dates['date_allocation_max'])
...
def extend_date_range(df, date_var, periods, type_aggregation, date_max):
    ...
    df_tail = df.copy()
    df_tail = df_tail.loc[df_tail[date_var] <= date_max]
    last_date = datetime.strptime(df_tail[date_var].iloc[-1], '%Y-%m-%d')
...
class BudgetAllocator(models.Model):
    mmm_dashboard = models.ForeignKey(
        MmmDashboard, on_delete=models.CASCADE, related_name="mmm_dashboard"
    )

    mmm_dashboard_validation = models.ForeignKey(
        MmmDashboard,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="mmm_dashboard_validation",
    )

    name = models.CharField(max_length=100, null=True, blank=True)
    reallocated_spend = models.FileField(null=True, blank=True)
    channel_constr_low_by_user = models.JSONField(null=True, blank=True)
    channel_constr_low_sorted = models.JSONField(null=True, blank=True)
    is_user_constr_low = models.BooleanField(default=False, null=False)
    channel_constr_up_by_user = models.JSONField(null=True, blank=True)
    channel_constr_up_sorted = models.JSONField(null=True, blank=True)
    is_user_constr_up = models.BooleanField(default=False, null=False)
    expected_spend = models.FloatField(null=True, blank=True)
    expected_spend_days = models.IntegerField(
        default=14, null=True, blank=True)
    target_value = models.FloatField(null=True, blank=True)
    flexibility = models.FloatField(default=30, null=True, blank=True)
    budget_date_start_by_user = models.DateField(null=True, blank=True)
    budget_date_end_by_user = models.DateField(null=True, blank=True)
    xtol_rel = models.FloatField(default=1.0e-10, null=True, blank=True)
    maxeval = models.IntegerField(default=700, null=True, blank=True)
    algoritm = models.CharField(
        default="LN_COBYLA", max_length=15, null=True, blank=True
    )
    note = models.TextField(null=True, blank=True)
    validation = models.JSONField(null=True, blank=True)

    incremental_prophet = models.JSONField(null=True, blank=True)
    ...
    def save_reallocated_spend(self, df_reallocated_spend):
        """
        Saves the DataFrame `df_reallocated_spend` as a CSV file to the `reallocated_spend` field of this `BudgetAllocator` instance.

        Parameters:
        - df_reallocated_spend (pandas.DataFrame): The DataFrame to save as a CSV file.
        """
        # Convert the DataFrame to CSV content
        content = df_reallocated_spend.to_csv(index=False)
        content_file = ContentFile(content.encode("utf-8"))

        # Generate a file name based on some unique attribute, e.g., ID or name
        file_name = (
            f"reallocated_spend_{self.id}_{str(time.time()).replace('.', '_')}.csv"
            if self.id
            else "reallocated_spend_temp.csv"
        )

        # Save the CSV content to the `reallocated_spend` field
        self.reallocated_spend.save(file_name, content_file)
        self.save()

    def save_from_dict(self, data_dict):
        """
        Updates the BudgetAllocator instance from a dictionary of parameters and optionally handles related DataFrame.

        Parameters:
        - data_dict (dict): Dictionary containing configurations, parameters, and analysis results.
        - reallocated_spend (DataFrame, optional): DataFrame containing reallocated spend data.
        """
        for key, value in data_dict.items():
            if key == "reallocated_spend":
                self.save_reallocated_spend(value)
            else:
